...
create_bar_plot <- function(data, target_metric, y_lims) {
  plot_df <- data %>% 
    filter(Metric == target_metric, TestSet == "A549") %>%
    mutate(
...
# 3. 生成并组合 (仅保留 4 个核心子图)
# ==========================================
p_auc  <- create_bar_plot(df_long, "AUC",  c(0.65, 0.95))
p_acc  <- create_bar_plot(df_long, "Acc",  c(0.70, 0.90))
p_f1   <- create_bar_plot(df_long, "F1",   c(0.50, 0.80))
...
# 4. 保存 (由于子图变少，调整了宽高比)
# ==========================================
ggsave("DNase_4Metrics_A549.pdf", final_plot, width = 10, height = 10, dpi = 300)
print(final_plot)