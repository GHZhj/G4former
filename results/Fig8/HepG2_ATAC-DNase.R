...
create_bar_plot <- function(data, target_metric, y_lims) {
  plot_df <- data %>% 
    filter(Metric == target_metric, TestSet == "HepG2") %>%
    mutate(
...
# 3. 生成并组合 (仅保留 4 个核心子图)
# ==========================================
p_auc  <- create_bar_plot(df_long, "AUC",  c(0.70, 1.00))
p_Acc  <- create_bar_plot(df_long, "Acc",  c(0.70, 1.00))
p_f1   <- create_bar_plot(df_long, "F1",   c(0.45, 0.90))
p_aupr <- create_bar_plot(df_long, "AUPRC", c(0.60, 0.95))
...
# 4. 保存 (由于子图变少，调整了宽高比)
# ==========================================
ggsave("DNase_4Metrics_HepG2.pdf", final_plot, width = 10, height = 10, dpi = 300)
print(final_plot)